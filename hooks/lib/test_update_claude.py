...
if [ "$1" = "plugin" ] && [ "$2" = "uninstall" ]; then
  printf '%s\\n' '{"version":2,"plugins":{}}' > "$ADW_STUB_CONFIG/plugins/installed_plugins.json"
  printf '%s\\n' '{}' > "$ADW_STUB_CONFIG/settings.json"
  exit 0
fi
...
if [ "$ADW_STUB_STALE_UPDATE" = "1" ] && [ "$1" = "plugin" ] && [ "$2" = "update" ]; then
  exit 0
fi
...
def test_stale_native_update_reinstalls_user_plugin(tmp_path: Path) -> None:
    home, source, config, cache, log, environment = _scenario(tmp_path)
    registry_path = config / "plugins" / "installed_plugins.json"
    registry = json.loads(registry_path.read_text(encoding="utf-8"))
    entry = registry["plugins"][update_claude.PLUGIN_ID][0]
    entry["gitCommitSha"] = "f" * 40
    entry["version"] = "f" * 12
    registry_path.write_text(json.dumps(registry), encoding="utf-8")
    old_cache = cache / "hooks" / "hooks.txt"
    old_content = old_cache.read_bytes()
    environment["ADW_STUB_STALE_UPDATE"] = "1"

    update_claude.install_pinned_plugin(home, source, COMMIT, environment)

    repaired = json.loads(registry_path.read_text(encoding="utf-8"))
    repaired_entry = repaired["plugins"][update_claude.PLUGIN_ID][0]
    assert repaired_entry["scope"] == "user"
    assert repaired_entry["gitCommitSha"] == COMMIT
    assert (config / "settings.json").exists()
    assert json.loads((config / "settings.json").read_text(encoding="utf-8"))["enabledPlugins"][update_claude.PLUGIN_ID] is True
    assert old_cache.read_bytes() == old_content
    calls = log.read_text(encoding="utf-8").splitlines()
    assert any(line.startswith("plugin uninstall ") and "--scope user --keep-data --yes" in line for line in calls)
    assert any(line.startswith("plugin install ") and "--scope user --yes" in line for line in calls)

...
from lib.shell_parse import (
    WRITE_REDIRECT_RE, INTERPRETERS,
    _bare, _basename, _command_word_index, _expand_home, _is_file_target, _is_quoted,
    _leading_assignments, _logical_lines, _segment_paths, _segment_text, _segments, _words, _write_paths,
...
def _mutation_targets(segment: list[str]) -> list[str]:
    ...
    raw_args = [token for token in segment[index + 1:] if not _bare(token).startswith("-")]
    if verb == "dd":
        targets.extend(_expand_home(_bare(token)) for token in raw_args if _bare(token).startswith("of="))
    elif verb in DESTINATION_LAST_ARG_VERBS and raw_args:

...
- Claude presets are `mixed`, `luna`, `haiku`, and `sonnet`. `/adw-judge status` reports the effective preset. Claude remote Desktop or Cowork installations default to `haiku`. Local Claude Code defaults to `mixed`. Codex always uses Luna and has no fallback.
- The Luna preset in Claude falls back to the matching Claude native agent when Codex subscription judging is unavailable. The fallback is Haiku for comments and Sonnet for prose/documents.
...
Replace Claude subprocess model calls with generated native hook settings. The plugin manifest retains deterministic command hooks. ADW owns a clearly identifiable generated block in Claude settings for model-backed review and rewrites only that block.

Add `/adw-judge mixed|luna|haiku|sonnet|status`. The command validates the preset, writes it atomically, regenerates the managed hook block, and prints whether reload is required. Native Claude profiles use `type: "agent"` on post-write and `Stop` lifecycles. `mixed` runs Haiku for comment judgment after eligible writes and Sonnet once at `Stop` for batched prose/document candidates. `haiku` and `sonnet` use the chosen Claude model for both roles. `luna` routes both roles to Task 2 and applies the specified Claude fallback only when Luna is unavailable.

Candidate journals prevent a Stop agent from rereading unrelated files and deduplicate repeated edits by final content hash. Agent responses are advisory findings consumed by the existing blocker flow, never direct pre-write denials. Tests execute configuration against temporary settings, validate idempotent switching and unrelated-hook preservation, verify role timing, fallback behavior, remote default selection, and malformed native-agent continuation.

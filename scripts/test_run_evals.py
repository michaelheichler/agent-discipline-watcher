...
def _stub_runner_config(tmp_path: Path, marker: Path) -> Path:
...
def _stub_runner_args(tmp_path: Path, marker: Path) -> argparse.Namespace:
    return argparse.Namespace(
        cases=ROOT / "evals" / "cases.jsonl",
        runner_config=_stub_runner_config(tmp_path, marker),
        runner="stub",
        condition="baseline",
...
class EvaluationHarnessTest(unittest.TestCase):
    def test_case_catalog_is_valid_and_balanced(self) -> None:
    ...
    def test_score_summary_applies_weights_and_release_gates(self) -> None:
        scores = []
    ...
    def test_candidate_blocker_fails_release_gate(self) -> None:
        rows = []
    ...
    def test_conditions_judged_on_different_cases_are_rejected(self) -> None:
        rows = [
    ...
    def test_duplicate_score_rows_are_rejected(self) -> None:
        rows = [
    ...
    @staticmethod
    def _score_row(case_id: str, condition: str, value: int) -> ScoreRow:
        return {
            "case_id": case_id,
            "trial": 1,
            "condition": condition,
    ...
    def test_jsonl_loader_reports_invalid_rows(self) -> None:
    ...
    def test_unmetered_runner_is_rejected_before_any_call(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            marker = tmp_path / "ran"
            args = _stub_runner_args(tmp_path, marker)
    ...
    def test_completed_keys_support_resuming_partial_runs(self) -> None:
        rows = [
            {
                "case_id": "direct-answer",
                "trial": 1,

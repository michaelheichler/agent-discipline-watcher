...
Only write-time regex enforcement remains. PreToolUse prevents known slop before a write. PostToolUse immediately blocks forced findings visible after a write. No watcher Stop or Pi agent-end callback runs.

## Requirements

1. Remove watcher Stop registrations from Claude and Codex while preserving unrelated Stop hooks.
2. Remove the Stop runtime, ledger, and Pi agent-end fallback.
...
2. The runtime contains no watcher Stop entry point, gate, ledger, or Pi agent-end handler.
